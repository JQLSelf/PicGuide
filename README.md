...
| 仪表盘 | 文件大小占比饼图、城市分布柱状图、标签词云、统计卡片 |
| 媒体扫描 | MD5 去重、EXIF 提取、缩略图缓存、增量归档、**Isolate 多核并行扫描** |
| GPS 反查 | 离线省市县反查（基于内置 GeoJSON）、全库重新分析、在线兜底 |
| **缩略图加载** | **统一 PixelThumb 组件、双清晰度策略（停留 20 秒升级原图）、视口热区懒加载、ImageCache LRU** |
...
| 设计点 | 说明 |
|--------|------|
| **双清晰度** | 初始 / 快速滚动：`cacheWidth: 300`（缩略图），停留 ≥ 20 秒后自动升级 `cacheWidth: 1920`（原图） |
